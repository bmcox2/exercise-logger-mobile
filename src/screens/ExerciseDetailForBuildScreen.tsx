...
type Props = NativeStackScreenProps<BuildStackParamList, "ExerciseDetailForBuild">;

const capitalize = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);

export default function ExerciseDetailForBuildScreen({ route, navigation }: Props) {
  const [exercise, setExercise] = useState<ExerciseLibraryRow | null>();
  const { exerciseId, onSelect, sourceWorkoutId } = route.params;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Pressable
        ...
        onPress={() => {
          onSelect(exercise);
          navigation.navigate("BuildWorkout", { sourceWorkoutId });
        }}
      >
        <Text style={styles.selectButtonText}>Select This Exercise</Text>
      </Pressable>
    </ScrollView>
  );
}
